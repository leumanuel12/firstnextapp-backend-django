...
def customer(request, id):
    ...
    if request.method == 'GET':
        ...
        serializer = CustomerSerializer(data, data=request.data) #(data, data=request.data) -- 1st parameter is referring to data from database, 2nd is for the data to replace/update the first
        if serializer.is_valid():
            serializer.save()
            return Response({'customer': serializer.data}, status=status.HTTP_200_OK)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
